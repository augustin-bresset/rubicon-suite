...
# Usage: ./ops/server/add_vpn_user.sh <first_last>
# Example: ./ops/server/add_vpn_user.sh john_doe
#
...
if [ ! -f "$WG_CONF" ]; then
  echo "WireGuard not configured. Run first: ./ops/server/setup_wireguard.sh"
  exit 1
fi
...
echo ""
echo "See ops/server/VPN_GUIDE.md for client installation instructions."
