...
# Usage: ./ops/server/setup_wireguard.sh
#
# Prerequisites: Ubuntu/Debian, sudo access, network interface eth0 (adjust if needed)
# After installation: add users with ./ops/server/add_vpn_user.sh <name>
...
# ── Add user peers below (see ops/server/add_vpn_user.sh) ──
CONF
...
echo ""
echo "Next steps:"
echo "  1. Configure UFW: ./ops/server/setup_firewall.sh prod"
echo "  2. Add users: ./ops/server/add_vpn_user.sh <first_last>"
echo "  3. Send QR code configs to employees (see ops/server/VPN_GUIDE.md)"
