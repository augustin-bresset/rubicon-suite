...
# Usage: ./ops/server/start_demo.sh
...
if [ -f "$CONF_FILE" ]; then
  ...
  if [ -z "$ADMIN_PASSWD_VAL" ] || [ "$ADMIN_PASSWD_VAL" = "CHANGE_ME" ]; then
    ...
    echo "Run first: ./ops/server/harden_demo.sh --restart"
    echo ""
    read -p "Continue anyway? [y/N] " -n 1 -r
    echo ""
    if [[ ! $REPLY =~ ^[Yy]$ ]]; then
      echo "Cancelled. Run ./ops/server/harden_demo.sh to secure the configuration."
      exit 1
    fi
  fi
fi
