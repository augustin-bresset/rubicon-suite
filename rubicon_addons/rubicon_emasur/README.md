...
1. Load the official codes into the `emasur_code` fields of `pdp.product`,
...
3. Reindex the history: run `action_backfill_design_emasur()` on
...
5. The PDP workspace filters client-side on `code`: extend its loaded fields
   with `emasur_code` when switching for real.
